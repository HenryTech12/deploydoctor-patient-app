...
> `DATABASE_URL` is hardcoded to `db-primary-internal` instead of the db
> service's real resolved private hostname, so `/db-ping` fails to resolve
> it. Deploy this branch to re-break the patient for a Network & VPN-category
> diagnosis.
